...
w2 = 10.0  # weight for distance cost Cd
...
def transition(d, theta, v, omega, d_values, theta_values):
    ...
    d_next = np.sqrt(max(d**2 - 2 * d * np.cos(theta - THETA_G) * v * delta_t + (v**2 * delta_t**2), 0))  # ensure non-negative distance
...
plt.grid(visible=True, linestyle='--', linewidth=0.5, color='black', alpha=0.3)
plt.xlabel('Distance to goal (m)')
plt.ylabel('Orientation (rad)')
plt.title('Optimal Value Function with Heatmap')
plt.xticks(d_values, rotation=45)  # Ensure grid aligns with discretized values
plt.yticks(theta_values)
...
while not abs(theta_traj[-1]) <= 1e-6 and step < max_steps:  # Continue until reaching the terminal state d = 0 or max steps reached
    min_value = np.inf
...
plt.quiver(d_traj[:-1], theta_traj[:-1], np.diff(d_traj), np.diff(theta_traj), angles='xy', scale=1, scale_units='xy', color='black')
