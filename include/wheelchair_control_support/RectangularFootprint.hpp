...
public:
    ...
    */
    ...
    bool check_point_inside_footprint(const double x, const double y)
    {
        ...
        if(x_wheelchair >= (-width_/2 - SMALL_NUMBER)
        && x_wheelchair<=(width_/2 +SMALL_NUMBER)  
        && y_wheelchair >= (-height_/2 - SMALL_NUMBER)
        && y_wheelchair <= (height_/2 + SMALL_NUMBER))
            return true;
     ...
     /**Return the distance from footprint to a point
    Return -1 if point inside footprint
    **/
    double distance_from_point_to_footprint(const double x, const double y)
    {
        double distance = square_distance_from_point_to_footprint(x, y);
        if (distance < 0)
        {
            // If the point is inside the footprint, return 0
            return -1;
        }
        else
        {
